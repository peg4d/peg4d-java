...
class FlowGraph  {
	...
	FlowGraph(PExpression e, FlowGraph inner, FlowGraph next) {
		this.e = e;
		this.inner = inner;
		this.next = next;
	}
	...
	void dump() {
		stack(new UList<PExpression>(new PExpression[256]), null, true);
	}
	
	void stack(UList<PExpression> stack, UList<PExpression[]> list, boolean mainPath) {
		int pos = stack.size();
//		System.out.print("N<" + note(e) + "> ");
	...
	private void buffer(UList<PExpression> stack, UList<PExpression[]> list) {
		if(list == null) {
			System.out.print("\t");
	...
	private String note(PExpression e) {
	...
	private boolean checkRecursion(PExpression e, UList<PExpression> stack) {
...
//	void checkExpression(PegRule rule, UList<PExpression> stack) {
...
//	}
//	
//	void checkNot(PegRule rule, PNot e, FlowGraph inner,  UList<PExpression> stack) {
//		stack.clear(0);
...
//	}
	...
	public static FlowGraph makeFlowGraph(String uniqueName, PExpression e, UMap<FlowGraph> buffer) {
	...
	private static FlowGraph makeGraph(PExpression e, FlowGraph tail, UMap<FlowGraph> rule) {
