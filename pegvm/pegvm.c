...
#if PEGVM_DEBUG
    assert(
        context->object_stack_pointer >= context->object_stack_pointer_base &&
        context->object_stack_pointer <
            &context
                 ->object_stack_pointer_base[PARSING_CONTEXT_MAX_STACK_LENGTH]);
#endif
    ...
    *context->call_stack_pointer++ = pc;
    ...
    assert(
        ...
        context->call_stack_pointer <
            &context
                 ->call_stack_pointer_base[PARSING_CONTEXT_MAX_STACK_LENGTH]);
#endif
}
...
#undef DEFINE_TABLE
    ...
    P4D_setObject(context, &context->left,
                  P4D_newObject(context, context->pos, pool));
