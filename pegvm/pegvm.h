...
#define PEGVM_OP_EACH(OP)                                                      \
  OP(EXIT) OP(JUMP) OP(CALL) OP(RET) OP(CONDBRANCH) OP(REPCOND) OP(CHARRANGE)  \
      ...
      OP(NOTBYTE) OP(NOTANY) OP(NOTCHARSET) OP(NOTBYTERANGE) OP(NOTSTRING)     \
      OP(ANDBYTE) OP(ANDCHARSET) OP(ANDBYTERANGE) OP(ANDSTRING)                \
      OP(OPTIONALBYTE) OP(OPTIONALCHARSET) OP(OPTIONALBYTERANGE)               \
